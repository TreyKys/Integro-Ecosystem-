...
// --- DUPLICATED USSD FUNCTIONS ---

exports.createAccount_ussd = onRequest({ secrets: [hederaAdminAccountId, hederaAdminPrivateKey] }, (request, response) => {
  cors(request, response, async () => {
    if (request.method !== "POST") {
      return response.status(405).send("Method Not Allowed");
    }
    try {
      ...
      console.log("createAccount_ussd: generated ECDSA privateKey (hex):", newPrivHex0x);
      ...
      console.log("createAccount_ussd: created accountId:", newAccountId.toString());

      const evmAddress = (new ethers.Wallet(newPrivHex0x)).address;
      console.log("createAccount_ussd: derived evmAddress (from ECDSA key):", evmAddress);
      ...
      console.error("FATAL ERROR in createAccount_ussd function:", error);
      return response.status(500).send({ error: error.message });
    }
  });
});

exports.executeNativeNftTransfer_ussd = onRequest({ secrets: [hederaAdminAccountId, hederaAdminPrivateKey] }, (request, response) => {
  cors(request, response, async () => {
    if (request.method !== "POST") {
      return response.status(405).send("Method Not Allowed");
    }
    try {
      ...
      console.error("ERROR in executeNativeNftTransfer_ussd:", error);
      return response.status(500).send({ error: error.message });
    }
  });
});

exports.mintRWAviaUSSD_ussd = onRequest({
  secrets: [hederaAdminAccountId, hederaAdminPrivateKey, hederaAdminSupplyKey]
}, (request, response) => {
  cors(request, response, async () => {
    if (request.method !== "POST") {
...
exports.setUserProfile_ussd = onRequest((request, response) => {
  cors(request, response, async () => {
    if (request.method !== "POST") {
      return response.status(405).send("Method Not Allowed");
    }
    ...
    try {
      ...
      console.error("ERROR in setUserProfile_ussd function:", error);
      return response.status(500).send({ error: error.message });
    }
  });
});
